...
export function LaunchContext ({ provider, cwd, prompt, goalsSnapshot, onChange, onCreateGoal }) {
  ...
  return (
    <section className="launch-context" data-state={state}>
      ...
      {editing && <div className="launch-context__fields">
        ...
        {onCreateGoal && <div className="launch-context__create">{creatingGoal ? <><input value={goalTitle} autoFocus onChange={(event) => setGoalTitle(event.target.value)} placeholder="New goal name" /><button type="button" disabled={!goalTitle.trim()} onClick={createGoal}>Create &amp; link</button><button type="button" onClick={() => setCreatingGoal(false)}>Cancel</button></> : <button type="button" onClick={() => setCreatingGoal(true)}>＋ Create a goal</button>}</div>}
      </div>}
    </section>
  )
}
...
export function ThreadContextPanel ({ sessionId, thread, goalsSnapshot }) {
  ...
  return (
    <>
      ...
      <section className="capsule-card"><h3>Session capsule <span>{tokenCount ? `${tokenCount} tokens` : ''}</span></h3>{binding?.capsuleText ? <details><summary>Preview frozen context</summary><pre>{binding.capsuleText}</pre></details> : <p className="context-muted">No saved capsule is available for this session yet.</p>}{binding?.capsuleHash && <code title={binding.capsuleHash}>{binding.capsuleHash.slice(0, 12)}…</code>}<small>New memories stay available through recall. Ambientic does not silently rewrite this session’s opening context.</small></section>
    </>
  )
}
...
export function MemoryWorkspace () {
  ...
  const load = async (search = '') => {
    ...
    try {
      const [memoryResult, projectResult, auditResult] = await Promise.all([
        ...
        typeof api.audit.list === 'function' ? api.audit.list({ types: ['memory', 'recall', 'capsule'], limit: 12 }) : []
      ])
  ...
  useEffect(() => { void load() }, [])
  useEffect(() => { const timer = setTimeout(() => { if (state !== 'unavailable') void load(query.trim()) }, 250); return () => clearTimeout(timer) }, [query])
  ...
  return <section className="memory-page"><header className="memory-topbar"><div><span>Memory</span><h1>What your agents can carry forward</h1><p>Local, scoped context shared across providers only when it is relevant.</p></div><form onSubmit={remember}><input value={newMemory} onChange={(event) => setNewMemory(event.target.value)} placeholder="Remember a preference or constraint…" aria-label="New memory" /><button type="submit" disabled={!newMemory.trim() || typeof contextApi().memory.remember !== 'function'}>Remember</button></form></header>
    ...
    </main><aside><section className="memory-profile"><span>User profile</span><b>{records.filter((item) => item.scope === 'user' && item.status === 'active').length}</b><p>Durable preferences and constraints available across providers.</p></section><section><header><b>Projects</b><span>{projects.length}</span></header>{projects.slice(0, 8).map((project) => <div className="memory-project" key={project.id}><i /><div><b>{project.name || project.brief}</b><small>{project.brief || project.rootPath || 'No folder required'}</small></div></div>)}{!projects.length && <p className="context-muted">Projects appear as Ambientic learns your working spaces.</p>}</section><section data-attention={conflicts > 0}><header><b>Needs attention</b><span>{conflicts}</span></header><p>{conflicts ? 'Conflicting or sensitive candidates are waiting for your decision.' : 'No conflicts. Quiet updates stay in the activity feed.'}</p></section><section><header><b>Recent activity</b><span>{activity.length}</span></header>{activity.slice(0, 6).map((item, index) => <div className="memory-activity" key={item.id || index}><i /><div><b>{item.title || item.type || 'Memory updated'}</b><small>{item.provider || item.session?.provider || ''} {formatRelativeTime(item.createdAt || item.timestamp)}</small></div></div>)}</section></aside></div>
  </section>
}
