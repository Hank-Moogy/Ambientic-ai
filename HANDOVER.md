...
- The first installed release attempt exposed an `@electron/osx-sign` mismatch: it applied distribution-style hardened runtime plus secure timestamps to a local Apple Development build, then rejected an untimestamped Electron resource. `release:local` now calls the signer's API with `timestamp: 'none'` and `hardenedRuntime: false`, while still requiring the stable Apple identity, rejecting ad-hoc output, and running strict deep verification.
