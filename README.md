...
- Every thread receives one short human-readable Ambientic name from its first substantive request. Provider context envelopes and other launch metadata are stripped before naming, including truncated index previews; later provider title changes are ignored. Threads can then be renamed from their header, and that user alias is stored against the stable provider thread ID and remains authoritative in Overview, Threads, the compact controller, and MIDI pads across provider refreshes and app restarts.
...
- Separate workspace and hardware indexes: dormant history appears in the desktop workspace, while the available hardware pads are assigned on launch to the most recently active real threads with attention first. Empty discovery placeholders remain dark, Codex managed/Desktop identities are deduplicated, and assigned positions stay fixed for the app run so recency never moves a thread under the user's hand.
...
- [x] Cross-provider handoff now resolves the canonical Ambientic project when a provider thread was launched from a broad directory, permits explicit handoff from real projects inside macOS-protected collections, regenerates the brief at click time, and surfaces actionable project-binding/provider errors instead of silently closing the picker.
...
- [x] Native approval cards now offer **Deny**, **Allow once**, **Allow for this thread**, and **Always allow** inside the active Ambientic thread when the provider can honor each scope. Managed Codex explicitly routes approval review to the user; app-server receives one-time/session decisions and persistent exec/network policy amendments when Codex proposes them. Claude’s official blocking hooks return one-time decisions, rewrite a permission suggestion to the in-memory `session` destination for thread grants, and preserve the provider’s persistent destination only for **Always allow**. Unsupported scopes are omitted instead of silently collapsing to another answer. If Ambientic is unavailable or times out, Claude falls back to its native permission behavior.
...
- [x] Ambientic-brokered tool approvals can offer three affirmative scopes — **Allow once**, **Allow for this thread**, and **Always allow** — and the grant each creates is coarser than the request that produced it, because approving one file and then being asked about its neighbour is what teaches people to click through prompts without reading them. A shell command is anchored to the folder it runs in, so "always allow Bash here" is expressible without becoming "Bash anywhere". `always` Ambientic grants are persisted and apply to every managed thread/provider; `session` grants are deliberately never written to disk. Native provider requests use their own truthful capabilities instead of pretending every provider supports the same persistence mechanism. A read-only grant never authorises a change, and no grant can widen into the home folder. Settings → Permissions lists every Ambientic standing grant with what it covers and a revoke that takes effect immediately.
...
- [x] Overview shows the controller's own grid instead of a thread mosaic. Each pad carries the thread's name in large bold type, lights in the same language as the hardware, and uses the session store's stable `padIndex`. On launch, real activity selects the available pads; empty process placeholders are excluded, and later activity fills vacancies without moving existing assignments. Screen and hardware resolve selection and meaning through the same store plus `src/shared/pad-light.mjs`, so the two cannot drift. The mosaic component, its styles, and its responsive rules are gone.
...
- [ ] Physically validate Codex and Claude approval cards against real one-time, thread, and provider-persistent permission requests; confirm that each provider shows only the scopes present in its live request.
...
- 2026-08-27 main-branch convergence: the current GitHub `main` history and `feature/ambientic-context-platform` were merged rather than replacing either line of work. Conflict resolution in the session store preserves both the main branch's stale-running lifecycle recovery and the feature branch's stable pad assignment, provider-identity deduplication, authoritative human thread names, and approval state behavior. The combined local-release suite passes 301/303 tests with 2 intentional transport skips and 0 failures; `git diff --check` is clean.

- 2026-08-27 authoritative human thread names: naming now runs for managed Ambientic tasks as well as terminal hooks, rejects complete or truncated `<ambientic-context>` envelopes as title material, reduces Codex/Claude/Hermes discovery titles to short human labels, and freezes the first assigned name against later provider-index changes. Manual renames migrate to a provider-stable Codex alias key so raw app-server and `codex-desktop:` identities share one override; the alias is also hydrated into the hardware session store, Codex transcript reads/completion reconciliation can no longer replace it with the provider's mutable name or preview, and a delayed inference result cannot race past a user rename. The local-release suite passes 270/272 with 2 intentional transport skips and 0 failures.

- 2026-08-27 hardware index and provider approvals: regressions prove that a managed Codex thread and its `codex-desktop:` discovery record collapse to one canonical session/pad; an idle discovered Hermes process with no real prompt receives no pad; latest activity establishes launch assignments; and the renderer follows the same stable `padIndex` as MIDI. Approval coverage proves Claude thread grants rewrite suggestions to the in-memory `session` destination, persistent Claude grants preserve the provider destination, Codex requests default to once/session, persistent exec/network policy is shown only when the provider proposes an amendment, permission-profile requests return only the requested subset at turn/session scope, and managed Codex turns explicitly use `approvalPolicy: on-request` with `approvalsReviewer: user`. The local-release suite passes 265/267 with its 2 intentional transport skips and the one documented OAuth simulator excluded; the ordinary full suite passed the other 264 tests present at that point and hit only that known OAuth timeout on its verification rerun. The focused final provider/hardware suite passes 52/52. The production main, preload, and renderer bundles build successfully; `git diff --check` is clean.

- 2026-08-27 Claude → Codex handoff repair: diagnostics reproduced the real failure immediately after Claude reported its session limit. Handoff eligibility now prefers the canonical bound project over a broad provider `cwd`, treats a specific project inside Documents as an explicit user-approved root without enabling background indexing, and blocks only genuinely broad or collection-level paths. An unbound thread opens the existing folder picker, creates or reuses the canonical project, rebinds the thread, and refreshes eligibility; provider or filesystem failures render as actionable alerts instead of disappearing. Focused project-scope/handover coverage passes 14/14; the full suite passes 261/263 runnable with 2 intentional transport skips and 0 failures, and the production Electron main, preload, and renderer bundles build successfully. Installed-app verification follows through the canonical local release gate.
