...
- [x] Restructured Workflow Studio around **Your workflows** and a separate **Workflow catalog**: installed routines, live run history, and pack results share one operational surface, while catalog cards install packs or navigate back to their installed workflows. Career OS now retains and displays every active role found by a scan by default, accepts any explicit display limit, and gives every opportunity a safe canonical/source **View job** action; the focused daily queue remains separate from the complete market evidence.
