...
- [ ] Re-run the complete installed Career OS first-run from a clean profile after the lifecycle repair: build and visibly review a profile from a real CV, LinkedIn PDF, selected imported provider memories, and company watchlist, then validate cross-source canonical resolution, deterministic remote eligibility, compensation confidence, complete scan visibility, source links and attribution, the time-boxed action queue, and the approval boundary before any application action.
- [ ] Run one deliberately interrupted live turn in Codex, Claude Code, and Hermes after installation to confirm the provider cards, Threads list, and APC40 MKII LEDs all leave Running on the documented fallback boundary.
...
- 2026-08-24 cross-provider thread-status repair: the actual Career OS Codex rollout contained several unmatched `task_started` events from interrupted turns, reproducing the permanently-running state. New lifecycle regressions cover stale unmatched Codex starts, explicit completion precedence, authoritative provider-native idle correction, and stale hook expiry for Codex, Claude Code, and Hermes without clearing waiting/approval states. The focused 33-test lifecycle suite passes; after restoring `better-sqlite3` from the Electron ABI left by the interrupted package to the system-Node ABI, the stable local-release suite passes 248 tests with 2 intentional socket skips and 0 failures, all production Electron bundles build, and `git diff --check` is clean.
- 2026-08-21 Career OS first-run and Workflow Studio repair: audited all four persisted first-run Codex threads and reproduced the pre-first-message materialization race that falsely failed workflows while agents kept running. The focused lifecycle/profile/pack/gateway/UI suite passes 58 tests with 2 intentional socket skips; coverage includes first-turn startup, transient-error handling, start preconditions, profile normalization/review, locked daily queues, complete market-result visibility, arbitrary result limits, canonical job actions, catalog/installed-workflow separation, legacy pack upgrades, and agent-request handoff. The stable local-release suite passes 244 tests with 2 socket-only skips and 0 failures; the production Electron main, preload, and renderer bundles build successfully, and `git diff --check` is clean. In-app browser discovery was unavailable for the visual click-through, so installed-app smoke remains the packaging gate.
- 2026-08-21 Career OS evidence-profile increment: 29 focused profile/pack/state/gateway/upgrade/UI tests pass with 2 intentional socket skips; the production Electron main, preload, and renderer bundles build successfully and `git diff --check` is clean. The unfiltered 229-test run passes 226 with 2 skips and retains the single previously documented simulated Claude OAuth callback timeout, which is unrelated to Career OS and excluded only by the approved local-release gate.
- 2026-08-21 Career OS multi-source Scout MVP: 35 focused source/pack/state/gateway/provider/UI tests pass with 2 intentional socket skips. The stable local-release suite passes 225 runnable tests with 0 failures and 2 skips; the production Electron bundles and `git diff --check` pass. Read-only live smokes returned current normalized roles from Greenhouse, Ashby, Himalayas, Remotive, Jobicy, Remote OK, and We Work Remotely; Lever’s official API responded successfully but its own board had no matching product role at test time. The unfiltered suite retains only the previously documented, unrelated simulated Claude OAuth callback timeout.
