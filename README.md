...
- `npm test`: 94 tests passing, including a simulated browser-to-localhost Claude OAuth callback, post-login forced usage refresh, CoreMIDI client reuse, protected project-scope boundaries, build identity, inherited provider-session environment sanitization, transcript navigation, provider approvals, current/legacy Claude usage navigation, live Claude authentication truth, MIDI layouts, voice validation, and cross-provider workspace behavior.
