...
test('Workflow Studio can install Career OS through progressive private setup', () => {
  ...
  assert.match(studio, /WorkflowGroupCard/)
  assert.match(studio, /Related workflows/)
  assert.match(studio, /latest outcome first/i)
  ...
  assert.match(workflowStyles, /\.workflow-group-card/)
  assert.match(workflowStyles, /\.workflow-group-header/)
})
